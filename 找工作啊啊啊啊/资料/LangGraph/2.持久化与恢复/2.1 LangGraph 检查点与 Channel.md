...
**5. Q：** 一个节点有多条出边（并行分支）时，superstep 怎么跑？
**A：** 一个节点的多条出边指向的所有目标节点，作为下一个 superstep 的一组并行任务同时执行；分支汇合时，下游节点等所有入边的并行操作完成、写入都生效后才运行。这也是「LastValue 一个 superstep 只许写一次」约束的来源：同一轮并行节点各写同一个 LastValue channel 就是未定义冲突，并行分支要写同一个键，得用带合并策略的 channel（如 operator.add）收集各分支的写入。

**6. Q：** LastValue / add_messages / BinaryOperatorAggregate 之外，还有内置 channel 类型吗？
**A：** 运行时（Pregel）层还有两种。Topic 是可配置的发布-订阅 channel，可按去重或跨步累积（accumulate=True）收集全部写入；DeltaChannel（langgraph>=1.2，beta）每步只存增量而非全量值，适合写得频繁、值持续变大的 channel（如长会话的消息列表），让快照不再随线程长度线性增大——它的 reducer 是批量 reducer（一次收到本轮全部写入、必须满足结合律），且在值重建时才执行，不在写入时执行。
